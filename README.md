...
Run the Streamlit price estimator locally:

```bash
# After completing notebooks 01-03, build the model artifacts once:
...
The app has three tabs: **Price Estimator** (suburb + land area → predicted price), **Suburb Trends** (interactive price history for any NSW suburb), and **Suburb Explorer** (most/least expensive suburbs, searchable table).
...
```
├── README.md
├── report.md             ← full technical report with embedded figures
├── app.py                ← Streamlit web app (price estimator + suburb explorer)
├── requirements.txt
├── .gitignore
│
├── data/
...
│
├── models/           ← generated by scripts/build_model.py, then committed
│   ├── xgb_model.json
│   ├── pipeline.pkl
│   ├── suburb_encoding.parquet
│   ├── suburb_annual_stats.parquet
│   ├── nsw_annual_stats.parquet
...
│
├── scripts/
│   └── build_model.py    ← exports model artifacts to models/ for the app
│
├── src/
